...
getwd()
setwd("C:/Users/brean/Downloads/masters/Fodor/builtenv")


#### read in files ####

missing_with_na <- read.csv("./cluster_runs/percent_missing/percent_missing_na_as_na.csv", row.name=1)
missing_no_na <- read.csv("./cluster_runs/percent_missing/percent_missing_na_as_na.csv",row.names = 1)


AUCs <- read.csv("./cluster_runs/associated_with_2192_renamed/AUCs/builtenv_AUCs.csv", colClasses = c("DEBIAS" = "factor"))
...
debias_weights_files <- list.files("./cluster_runs/associated_with_2192_renamed/DEBIAS-M_runs/",
                                   pattern = "debias_weights", full.names = TRUE)
phenos <- read.csv("./csv_files/phenotypes.csv")


pval.df <- read.csv("./cluster_runs/associated_with_2192_renamed/AUCs/builtenv_AUC_pvals.csv", colClasses = c("DEBIAS" = "factor"))
...
hist_pdf <- function(input_folder, pattern, output_folder){ 
  ...
  ggsave(output_folder, marrangeGrob(grobs=plots, nrow=2, ncol=2), width = 4, height = 4, dpi = 300)
}

# pre, ROC
hist_pdf("./cluster_runs/associated_with_2192_renamed/output/" ,"pre_DEBIAS-M_RF_lognorm_ROC", "./output/associated_with_2192_renamed/ROC_pre_100_perm.png")

# pre hist
hist_pdf("./cluster_runs/associated_with_2192_renamed/output/" ,"pre_DEBIAS-M_RF_lognorm_hist", "./output/associated_with_2192_renamed/hist_pre_100_perm.png")

# post, ROC
hist_pdf("./cluster_runs/associated_with_2192_renamed/output/" ,"post_DEBIAS-M_RF_lognorm_ROC", "./output/associated_with_2192_renamed/ROC_post_100_perm.png")

# post hist
hist_pdf("./cluster_runs/associated_with_2192_renamed/output/" ,"post_DEBIAS-M_RF_lognorm_hist", "./output/associated_with_2192_renamed/hist_post_100_perm.png")
...
t_auc = AUCs %>% filter(Permutation == 0) %>% select(DEBIAS, AUC) 
with(t_auc, t.test(AUC[DEBIAS == 0], AUC[DEBIAS == 1], alternative = "two.sided", paired = TRUE))

# next three lines are only needed if i have phenotypes to add...I don't
...
with(t_pval, t.test(pval[DEBIAS == 0], pval[DEBIAS == 1], alternative = "two.sided", paired = TRUE))
...
png("./output/associated_with_2192_renamed/box_AUC_pval.png", width = 1050, height = 480)
